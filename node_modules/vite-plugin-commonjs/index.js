...
function normalizePath$1(id) {
  return path.posix.normalize(isWindows$1 ? slash$1(id) : id);
}
...
class Resolve {
  ...
  async tryResolveAlias(importee, importer) {
    ...
    const relativePath = alias.replacement.startsWith(".") ? alias.replacement : relativeify(path$1.posix.relative(path$1.dirname(importer), alias.replacement));
    const resolvedAlias = {
      ...alias,
      findString,
  ...
  tryResolveBare(importee, importer) {
    ...
    while (p = paths.shift()) {
      ...
      if (fs.existsSync(fullPath)) {
        find = level;
        const relativePath = relativeify(path$1.posix.relative(path$1.dirname(importer), node_modules));
    ...
    const alias = {
      ...
      replacement,
      findString: find,
      relative: replacement.startsWith(".") ? replacement : relativeify(path$1.posix.relative(path$1.dirname(importer), replacement))
    };
    return {
      type: "bare",
  ...
  resolveAlias(importee, importer, alias) {
    ...
    if (replacement.startsWith(".")) {
      ...
      const relativePath = relativeify(path$1.posix.relative(
        path$1.dirname(importer),
...
class DynaimcRequire {
  ...
  async generateRuntime(analyzed) {
    ...
    for (const req of analyzed.require) {
      ...
      files = files.filter((f) => normalizePath$1(path.join(path.dirname(id), f)) !== id);
...
async function globFiles(node, code, importer, resolve, extensions, loose = true) {
  ...
  files = fastGlob.sync(fileGlobs, { cwd: path.dirname(importer) }).map((file) => relativeify$1(file));
